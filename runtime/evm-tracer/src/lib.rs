...
pub mod tracer {
	...
	impl EvmTracer {
		pub fn new() -> Self {
			Self {
				step_event_filter: fp_ext::moonbeam_ext::step_event_filter(),
			}
		}
		...
		pub fn emit_new() {
			fp_ext::moonbeam_ext::call_list_new();
		}
	}
	...
	impl EvmListener for EvmTracer {
		...
		fn event(&mut self, event: evm::tracing::Event) {
			...
			fp_ext::moonbeam_ext::evm_event(message);
		}
	}
	...
	impl GasometerListener for EvmTracer {
		...
		fn event(&mut self, event: evm_gasometer::tracing::Event) {
			...
			let message = event.encode();
			fp_ext::moonbeam_ext::gasometer_event(message);
		}
	}
	...
	impl RuntimeListener for EvmTracer {
		...
		fn event(&mut self, event: evm_runtime::tracing::Event) {
			...
			let message = event.encode();
			fp_ext::moonbeam_ext::runtime_event(message);
		}
	}
}
